...
public class MovelController {
    ...
    public ResponseEntity<Page<MovelResponse>> listar(PaginatedRequest paginatedRequest) {
        return ResponseEntity.ok(service.listarMoveis(paginatedRequest));
    }
    ...
    @GetMapping("/{nome}")
    public ResponseEntity<List<MovelResponse>> getByNome(@PathVariable String nome) {
        return ResponseEntity.ok(service.getMovelByNome(nome).stream().map(MovelResponse::new).toList());
    }
